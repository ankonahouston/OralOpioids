...
#' CanadianOralOpioids: Obtain the latest information on Morphine equivalent doses provided by HealthCanada
#' This package provides details on Oral Opioids approved for sale by Health Canada. Please note that the output generated by the package should not be substituted for clinical advise and any medication should be only consumed at the advise of a licensed healthcare provider.
#'
#' @docType package
#' @name CanadianOralOpioids
  NULL
...
#' If "", filelocation will be set to the download path within the CanadianOralOpioids
...
load_HealthCanada_Opioid_Table <- function(filelocation = "", no_download = FALSE, verbose = TRUE){
  ...
  if (filelocation == ""){
    filelocation <- paste0(system.file(package = "CanadianOralOpioids"),"/download")
  }
  ...
  if (HealthCanada_Opioid_Table_is_old == FALSE){
    ...
    if (downloadq == FALSE && HealthCanada_Opioid_Table_files_exist == FALSE){
      ...
      files <- lapply(list.files(system.file('extdata', package = 'CanadianOralOpioids'), full.names = TRUE), read.csv)
      ...
      comment(out) <- c(msg = out_msg,path=HealthCanada_Opioid_Table_path,Disclaimer= "Not a substitute for medical advise,
                        Source= https://www.cihi.ca/sites/default/files/document/opioid-prescribing-canada-trends-en-web.pdf.")
